...
let availability = {}; // Stores professionals' availability
    ...
    function deleteAvailability(professional) {
        // Get the selected date and time from the dropdown
        let dateTime = document.getElementById(`available-dates-${professional}`).value;
    
        // Check if the professional has any availability set
        if (!availability[professional] || availability[professional].length === 0) {
            alert(`${professional} has no availability to delete.`);
            return;
        }
        ...
        // Remove the selected date and time from the availability list
        availability[professional] = availability[professional].filter(slot => slot !== dateTime);
    
        // Reload the updated availability into the dropdown
        loadAvailability(professional);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    window.payWithPaystack = function(expert, price) {
        if (!availabilityChecked[expert]) {
            ...
            // Replace 'YOUR_FLUTTERWAVE_PAYMENT_LINK' with the actual payment link
            window.open("https://sandbox.flutterwave.com/pay/hhtgwl7los4p");
        }
    };
...
// Function to toggle the visibility of the booking log
function toggleBookingLog() {
    const logContainer = document.getElementById('booking-log-container');
    const toggleButton = document.querySelector('.toggle-log-button');
    if (logContainer.style.display === 'none' || logContainer.style.display === '') {
        logContainer.style.display = 'flex'; // Show the log
        toggleButton.textContent = 'Hide Booking Log';
    } else {
        logContainer.style.display = 'none'; // Hide the log
        toggleButton.textContent = 'Show Booking Log';
    }
}

// Function to process payment and generate unique booking details
function processPayment(mentor) {
    const nameInput = document.getElementById(`client-name-${mentor}`).value;
    const dateInput = document.getElementById(`date-${mentor}`).value;
    const timeInput = document.getElementById(`time-${mentor}`).value;

    if (!nameInput || !emailInput || !dateInput || !timeInput) {
        alert('Please fill all the fields before proceeding.');
        return;
    }

    // Simulate payment process
    alert(`Processing payment for ${nameInput}...`);

    // Simulate successful payment
    setTimeout(() => {
        alert('Payment confirmed!');
        const uniqueName = generateUniqueName(nameInput);
        const zoomLink = generateMeetingLink();
        addToBookingLog(uniqueName, emailInput, dateInput, timeInput, zoomLink);
    }, 2000);
}

// Function to add an entry to the booking log
function addToBookingLog(name, email, date, time, zoomLink) {
    const bookingLog = document.getElementById('booking-log');
    const newEntry = document.createElement('div');
    newEntry.classList.add('log-entry');

    newEntry.innerHTML = `
        <div class="log-item">${name}</div>
        <div class="log-item">${date}</div>
        <div class="log-item">${time}</div>
        <div class="log-item zoom-link"><a href="${zoomLink}" target="_blank">Join Meeting</a></div>
    `;

    bookingLog.appendChild(newEntry);
}
...
// Function to generate a pre-assigned meeting link
function generateMeetingLink() {
    const randomMeetingId = Math.floor(100000000 + Math.random() * 900000000); // Random 9-digit number
